...
sub emit_stop_cg_sequence {
    ## XXXX: using `previous_channel here is a problem
    my $ts = <<'END'
                        $stJ := 0
                        while ($stJ < 128)
...
END
    ##XXX: BROKEN BROKEN -- this doesn't actually replace [[STOP_CG_SEQUENCE]] with the body of emit_stop_cg_sequence
...
END
    ...
    my $ts = <<'END';
        case [[SEQ]]
            [[ALL_STOP]]
            [[STOP_MASTER_CLOCK]]
            ## XXX: STOP_CG_SEQUENCE will be made to stop the existing sequence (or a noop if AllStop was set)
            [[STOP_CG_SEQUENCE]]
            ...
            if (`use_master_clock = 0 or ($ENGINE_UPTIME-$master_clock_start) <= `responsive_delta)
                $stVelocity := $stVelocity + real_to_int(`seq_delta)                                              
                if ($stVelocity < 1)
                    $stVelocity := 1
                end if
                ...
                $stDoinit = 1
                while (`i < `end and $stDoinit = 1)
                    if (%n[4*`i] < `responsive_delta)                    
                        select (%n[4*`i])
                        case $MIDI_COMMAND_NOTE_ON
                            set_midi(`channel, %n[4*`i+1], %n[4*`i+2], $stVelocity)
                            %offs[`channel*128 + %n[4*`i+2]] := [[CG]]
